...
export const fetchUsersPaginated = createAsyncThunk(
  ...
  async (
    ...
    try {
      const response = await userApi.getUsersPaginatedApi(
        page,
        limit,
        search,
        unitId,
    ...
    try {
      const payload = {
        ...
        contactNo: data.phone
          ...
          : "0000000000",
        password: data.password, // default password required by backend
      ...
      return {
        ...
        phone: String(u.contactNo || u.phone || data.phone || ""),
      };
    } catch (err: any) {
  ...
  async ({ id, data }: { id: string; data: any }, { rejectWithValue }) => {
    try {
      ...
      const payload: any = {
        fullName: data.name,
        email: data.email,
        contactNo: (cleanPhone && cleanPhone !== "0") ? cleanPhone : null,
        role: data.designation,
        unit_id: data.unitId,
        team_id: data.teamId,
      };
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    // fetch
    builder.addCase(fetchUsersPaginated.pending, (state) => {
      state.loading = true;
      state.error = null;
    });
    builder.addCase(fetchUsersPaginated.fulfilled, (state, action) => {
      ...
      state.users = Array.isArray(rawData)
        ? rawData.map((u: any) => ({
            ...u,
            id: u._id || u.id,
        ...
        : [];
      if (action.payload.pagination?.totalRecords !== undefined)
        state.pagination.total = action.payload.pagination.totalRecords;
      if (action.payload.pagination?.currentPage !== undefined)
        state.pagination.page = action.payload.pagination.currentPage;
    });
    builder.addCase(fetchUsersPaginated.rejected, (state, action) => {
      state.loading = false;
