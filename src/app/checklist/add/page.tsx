...
export default function AddChecklist() {
  ...
  const { items, info, loaded, updateItem, setInfoField, saveRecord } =
    useChecklist();
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      <div className="bg-emerald-700 text-white px-4 py-3 flex items-center gap-3 sticky top-0 z-20 shadow-md">
        <button
          onClick={() => router.push("/checklist/records")}
        ...
        <div className="flex-1">
          ...
          <p className="text-xs opacity-80 mt-0.5">
            {displayVal(info.nameOf3PL, "Lotus Marketing")} ·{" "}
            {displayVal(info.location, "NIYOL")} ·{" "}
            {displayVal(info.month, "Apr-26")}
          </p>
        </div>
      </div>
      ...
      <div className="px-2.5 py-3 pb-10">
        <div className="bg-white rounded-tr-xl rounded-b-xl overflow-hidden shadow-md">
          <div className="bg-emerald-50 text-white px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
            <div>
              <p className="font-semibold text-emerald-50 text-emerald-700 text-sm">
                Secondary Warehouse – Self Assessment Checklist
              </p>
              <p className="text-xs opacity-80 text-emerald-700 mt-1">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-gray-100">
            <InlineField
              label="Name of 3PL"
              value={info.nameOf3PL}
              onChange={setInfoField("nameOf3PL")}
              placeholder="e.g. Lotus Marketing"
            />
            <InlineField
              label="Location"
              value={info.location}
              onChange={setInfoField("location")}
              placeholder="e.g. NIYOL"
            />
            <InlineField
              label="Person Met"
              value={info.personMet}
              onChange={setInfoField("personMet")}
              placeholder="e.g. Viresh Modi"
            />
            <InlineField
              label="Designation"
              value={info.designation}
              onChange={setInfoField("designation")}
              placeholder="e.g. Manager"
            />
            <InlineField
              label="Assessed By"
              value={info.assessedBy}
              onChange={setInfoField("assessedBy")}
              placeholder="e.g. Self"
            />
            <InlineField
              label="Date"
              value={info.date}
              onChange={setInfoField("date")}
              type="date"
            ...
            <div className="px-4 py-3">
              ...
              <div className="flex gap-2 items-center">
                <div className="flex-1">
                  ...
                  <input
                    type="date"
                    value={info.periodFrom}
                    onChange={(e) => setInfoField("periodFrom")(e.target.value)}
                ...
                <div className="flex-1">
                  ...
                  <input
                    type="date"
                    value={info.periodTo}
                    onChange={(e) => setInfoField("periodTo")(e.target.value)}
                    className="w-full text-xs font-semibold text-gray-800 border-b border-gray-200 outline-none focus:border-emerald-500 bg-transparent pb-0.5"
                  />
                </div>
              </div>
            ...
            <table className="w-full text-xs" style={{ minWidth: 680 }}>
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="px-2 py-2 text-left text-gray-500 font-semibold w-8">
                    #
                  </th>
                  <th className="px-2 py-2 text-left text-gray-500 font-semibold">
                    Particular
                  </th>
                  <th className="px-2 py-2 text-center text-gray-500 font-semibold">
                    Category
                  </th>
                  <th className="px-2 py-2 text-center text-gray-500 font-semibold">
                    Max
                  ...
                  <th className="px-2 py-2 text-left text-gray-500 font-semibold w-36">
                    Remarks
                  </th>
                </tr>
              </thead>
