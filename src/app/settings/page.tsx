...
import {
  ...
  updateMaster1,
  deleteMaster1,
} from "../../redux/slices/master1Slice";
import {
  fetchMaster2ByMaster1,
  addMaster2,
  updateMaster2,
  deleteMaster2,
...
export default function UnifiedMastersPage() {
  ...
  useEffect(() => {
    if (master1State.items) {
      setLocalMaster1(
        [...master1State.items].sort((a, b) => a.order - b.order),
      );
    }
  }, [master1State.items]);
  ...
  useEffect(() => {
    if (master2State.items) {
      setLocalMaster2(
        [...master2State.items].sort((a, b) => a.order - b.order),
      );
    }
  }, [master2State.items]);
  ...
  // ----- DRAWER STATES -----
  const [drawerType, setDrawerType] = useState<"master1" | "master2" | null>(
    null,
  );
  ...
  const [m1Errors, setM1Errors] = useState<{
    name?: string;
    weightage?: string;
  }>({});
  ...
  const [m2Errors, setM2Errors] = useState<{
    master1Id?: string;
    particulars?: string;
    category?: string;
    maxScore?: string;
  ...
  const saveMaster1 = async () => {
    ...
    if (!m1Form.weightage || isNaN(Number(m1Form.weightage)))
      ...
      if (editingId) {
        await dispatch(
          updateMaster1({
            id: editingId,
            data: {
              name: m1Form.name.trim(),
              weightage: Number(m1Form.weightage),
            },
          }),
        ).unwrap();
      } else {
  ...
  const saveMaster2 = async () => {
    ...
    if (!m2Form.maxScore || isNaN(Number(m2Form.maxScore)))
      e.maxScore = "Required";
    if (Object.keys(e).length) {
      setM2Errors(e);
      return;
      ...
      if (editingId) {
        await dispatch(
          updateMaster2({
            id: editingId,
            data: {
              master1Id: m2Form.master1Id,
              name: m2Form.particulars.trim(),
              category: m2Form.category.trim(),
              maxScore: Number(m2Form.maxScore),
            },
          }),
        ).unwrap();
      } else {
        const childCount = localMaster2.filter(
          (m2) => m2.master1Id === m2Form.master1Id,
        ).length;
        await dispatch(
          addMaster2({
            master1Id: m2Form.master1Id,
  ...
  const handleM1DragEnd = async () => {
    if (
      ...
      m1DragItem.current === m1DragOver.current
    )
      return;
    ...
    try {
      await Promise.all(
        reordered.map((item) =>
          dispatch(
            updateMaster1({ id: item.id, data: { order: item.order } }),
          ).unwrap(),
        ),
      );
    } catch (err) {
  ...
  const handleM2DragEnd = async (parentId: string) => {
    ...
    if (
      m2DragItem.current.parentId !== parentId ||
      m2DragOver.current.parentId !== parentId
    )
      return;
    ...
    const reorderedChildren = children.map((it, i) => ({
      ...it,
      order: i + 1,
    }));
    ...
    try {
      await Promise.all(
        reorderedChildren.map((item) =>
          dispatch(
            updateMaster2({ id: item.id, data: { order: item.order } }),
          ).unwrap(),
        ),
      );
    } catch (err) {
    ...
    <div className="flex flex-col h-screen w-full overflow-hidden bg-slate-50 font-sans text-xs">
      <div className="bg-emerald-700 text-white px-6 py-4 shadow-md shrink-0 flex items-center gap-3">
        <div>
          ...
          <p className="text-[11px] opacity-80 mt-0.5">
            Manage hierarchical Master 1 and Master 2 classifications
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto px-6 pt-5 pb-10">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={openAddMaster1}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 shadow-sm font-semibold text-white transition-colors text-[13px]"
          >
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="divide-y divide-gray-100">
            {master1State.loading && localMaster1.length === 0 ? (
              <div className="text-center py-14 text-sm text-slate-300 italic">
                Loading hierarchy...
              </div>
            ) : localMaster1.length === 0 ? (
              <div className="text-center py-14 text-sm text-slate-300 italic">
                No master categories. Click Add Master 1.
              </div>
            ) : (
              localMaster1.map((m1, idx) => {
                const isExpanded = !!expanded[m1.id];
                const m2Children = localMaster2.filter(
                  (m2) => m2.master1Id === m1.id,
                );
                const totalM2Score = m2Children.reduce(
                ...
                return (
                  <div key={m1.id} className="flex flex-col">
                    ...
                    <div
                      draggable
                      onDragStart={() => {
                        m1DragItem.current = idx;
                      }}
                      onDragEnter={() => {
                        m1DragOver.current = idx;
                      }}
                    ...
                    >
                      ...
                      <div className="px-4 py-3.5 flex items-center font-bold text-sm text-slate-800">
                        ...
                        {m1.name}
                        <span className="ml-3 text-[10px] font-semibold bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">
                          {m2Children.length} items
                        </span>
                      </div>
                      <div className="px-4 py-3.5 flex items-center justify-end text-sm text-slate-600 font-medium">
                        {m1.weightage}%
                      </div>
                      <div className="px-4 py-3.5 flex items-center justify-end gap-1 transition-opacity">
                        <button
                          onClick={() => openAddMaster2(m1.id)}
                          title="Add Master 2"
                            ...
                            <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                              {m2Children.length === 0 ? (
                                <div className="text-center py-6 text-[11px] text-slate-400 italic">
                                  No sub-items. Click (+) to add Master 2
                                  records.
                                </div>
                              ) : (
                                <table className="w-full text-left text-[11px]">
                                  ...
                                  <tbody className="divide-y divide-gray-50">
                                    {m2Children.map((m2, m2Idx) => (
                                      ...
                                      >
                                        ...
                                        <td className="px-4 py-2 text-emerald-600 font-semibold text-right">
                                          {m2.maxScore}
                                        </td>
                                  ...
                                  <tfoot className="bg-slate-50 border-t border-gray-100">
                                    <tr>
                                      <td
                                        colSpan={3}
                                        className="px-4 py-2 font-semibold text-slate-500 text-right"
                                      >
                                        Total Sub-Score
                                      </td>
                                      <td className="px-4 py-2 font-bold text-emerald-700 text-right">
                                        {totalM2Score}
                                      </td>
                                      <td></td>
                                    </tr>
        ...
        {drawerType && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 z-50 max-w-md w-full bg-white flex flex-col shadow-2xl"
            >
                    ...
                    <>
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          value={m1Form.name}
                          onChange={(e) => {
                            setM1Form((f) => ({ ...f, name: e.target.value }));
                            setM1Errors((er) => ({ ...er, name: "" }));
                          }}
                          onKeyDown={(e) => e.key === "Enter" && saveMaster1()}
                          placeholder="e.g. Technical Skills"
                          className={`w-full bg-gray-100 text-slate-800 placeholder-slate-400 border-0 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm ${m1Errors.name ? "ring-2 ring-red-400" : ""}`}
                        />
                        {m1Errors.name && (
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          type="number"
                          min="0"
                          value={m1Form.weightage}
                          onChange={(e) => {
                            setM1Form((f) => ({
                              ...f,
                              weightage: e.target.value,
                            }));
                            setM1Errors((er) => ({ ...er, weightage: "" }));
                          }}
                          placeholder="e.g. 20"
                          className={`w-full bg-gray-100 text-slate-800 placeholder-slate-400 border-0 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm ${m1Errors.weightage ? "ring-2 ring-red-400" : ""}`}
                        />
                        {m1Errors.weightage && (
                          <p className="text-xs text-red-500">
                            {m1Errors.weightage}
                      ...
                      <button
                        onClick={saveMaster1}
                        className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg font-semibold transition-colors text-sm mt-2"
                      >
                        Save Master 1
                      </button>
                    </>
                  ...
                  {drawerType === "master2" && (
                    <>
                      <div className="space-y-1">
                        ...
                        <select
                          value={m2Form.master1Id}
                          onChange={(e) => {
                            setM2Form((f) => ({
                              ...f,
                              master1Id: e.target.value,
                            }));
                            setM2Errors((er) => ({ ...er, master1Id: "" }));
                          }}
                          className={`w-full bg-gray-100 text-slate-800 border-0 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm ${m2Errors.master1Id ? "ring-2 ring-red-400" : ""}`}
                        >
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          type="text"
                          autoFocus={!!m2Form.master1Id}
                          value={m2Form.particulars}
                          onChange={(e) => {
                            setM2Form((f) => ({
                              ...f,
                              particulars: e.target.value,
                            }));
                            setM2Errors((er) => ({ ...er, particulars: "" }));
                          }}
                          onKeyDown={(e) => e.key === "Enter" && saveMaster2()}
                          placeholder="e.g. React.js Knowledge"
                          className={`w-full bg-gray-100 text-slate-800 placeholder-slate-400 border-0 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm ${m2Errors.particulars ? "ring-2 ring-red-400" : ""}`}
                        />
                        {m2Errors.particulars && (
                          <p className="text-xs text-red-500">
                            {m2Errors.particulars}
                          </p>
                        )}
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          type="text"
                          value={m2Form.category}
                          onChange={(e) => {
                            setM2Form((f) => ({
                              ...f,
                              category: e.target.value,
                            }));
                            setM2Errors((er) => ({ ...er, category: "" }));
                          }}
                          placeholder="e.g. Frontend Engineering"
                          className={`w-full bg-gray-100 text-slate-800 placeholder-slate-400 border-0 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm ${m2Errors.category ? "ring-2 ring-red-400" : ""}`}
                        />
                        {m2Errors.category && (
                          <p className="text-xs text-red-500">
                            {m2Errors.category}
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          type="number"
                          min="0"
                          value={m2Form.maxScore}
                          onChange={(e) => {
                            setM2Form((f) => ({
                              ...f,
                              maxScore: e.target.value,
                            }));
                            setM2Errors((er) => ({ ...er, maxScore: "" }));
                          }}
                          placeholder="e.g. 10"
                          className={`w-full bg-gray-100 text-slate-800 placeholder-slate-400 border-0 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm ${m2Errors.maxScore ? "ring-2 ring-red-400" : ""}`}
                        />
                        {m2Errors.maxScore && (
                          <p className="text-xs text-red-500">
                            {m2Errors.maxScore}
