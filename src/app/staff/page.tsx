...
const INITIAL_STAFF: Staff[] = [
  {
    id: "s-1",
    name: "Rahul Patel",
    email: "rahul.patel@corp.acme.com",
    phone: "+1 (555) 124-5678",
    designation: "VP of Sales & Revenue",
    unitId: "u-1",
    teamId: null,
  },
  {
  ...
  },
  {
    id: "s-3",
    name: "Priya Sharma",
    email: "priya.sh@corp.acme.com",
    phone: "+1 (555) 345-6789",
    designation: "Growth Creative Lead",
    unitId: "u-3",
    teamId: null,
  },
  {
    id: "s-4",
    name: "Amit Shah",
    email: "amit.shah@corp.acme.com",
    phone: "+1 (555) 456-7890",
    designation: "Sr. Representative Enterprise Sales",
    unitId: "u-1",
    teamId: "t-1",
  },
  {
    id: "s-5",
    name: "Rakesh Gupta",
    email: "rakesh.g@corp.acme.com",
    phone: "+1 (555) 567-8901",
    designation: "Partner Growth Manager",
    unitId: "u-1",
    teamId: "t-2",
  },
  {
    id: "s-6",
    name: "Manav Shah",
    email: "manav.shah@corp.acme.com",
  ...
  },
  {
    id: "s-7",
    name: "Jay Mehta",
    email: "jay.mehta@corp.acme.com",
    phone: "+1 (555) 789-0123",
    designation: "Core QA Architect",
    unitId: "u-2",
    teamId: "t-4",
  },
  {
    id: "s-8",
    name: "Neha Joshi",
    email: "neha.j@corp.acme.com",
    phone: "+1 (555) 890-1234",
    designation: "Principal Brand Designer",
    unitId: "u-3",
    teamId: "t-5",
  },
  {
    ...
    email: "saurabh.d@corp.acme.com",
    phone: "+1 (555) 901-2345",
    designation: "Account Executive Middle-market",
    unitId: "u-1",
    teamId: "t-1",
  },
  {
    id: "s-10",
    name: "Kiran Kumar",
    email: "kiran.k@corp.acme.com",
    phone: "+1 (555) 012-3456",
    designation: "React Platform Engineer",
    unitId: "u-2",
    teamId: "t-3",
  },
  {
    id: "s-11",
    name: "Ananya Roy",
    email: "ananya.roy@corp.acme.com",
    phone: "+1 (555) 123-0987",
    designation: "Manual QA Specialist",
    unitId: "u-2",
    teamId: "t-4",
  },
  {
    id: "s-12",
    name: "Siddharth Sen",
    email: "freelancer@corp.acme.com",
    phone: "+1 (555) 987-6543",
    designation: "Internal Operations Consultant",
    unitId: null,
    teamId: null,
  },
];
...
export default function StaffPage() {
  ...
  const { users: staff, pagination } = useSelector(
  ...
  const handleCreateUserInsidePicker = async (e: React.FormEvent) => {
    ...
    const newStaffItem = {
      ...
      email:
        pickerForm.email ||
        `${pickerForm.name.toLowerCase().replace(/\s+/g, ".")}@corp.acme.com`,
      phone: pickerForm.phone || "",
    ...
    try {
      ...
      if (drawer.payload?.onSelect && res?.id)
        drawer.payload.onSelect(res.id, res.name);
    } catch (err) {
  ...
  const handleSaveUnit = async (e: React.FormEvent) => {
    ...
    try {
      if (drawer.type === "add-unit") {
        await dispatch(
          addUnit({ name: unitForm.name, headId: unitForm.headId || null }),
        ).unwrap();
        dispatch(fetchUnits());
        ...
        await dispatch(
          updateUnit({
            id: drawer.targetId,
            data: { name: unitForm.name, headId: unitForm.headId || null },
          }),
        ).unwrap();
        dispatch(fetchUnits());
      ...
      if (drawer.type === "add-staff") {
        await dispatch(
          addUser({
            ...
            email:
              staffForm.email ||
              `${staffForm.name.toLowerCase().replace(/\s+/g, ".")}@corp.acme.com`,
        ...
        if (staffForm.phone?.trim()) {
          updateData.phone = staffForm.phone;
        }
        await dispatch(
          updateUser({ id: drawer.targetId, data: updateData }),
        ).unwrap();
        if (staffForm.designation === "unit_head") dispatch(fetchUnits());
        if (staffForm.designation === "team_head") dispatch(fetchTeams());
        dispatch(
  ...
  const staffFormTeams = useMemo(() => {
    ...
    return teams.filter((t: any) => t.unitId === staffForm.unitId);
  }, [teams, staffForm.unitId]);

  const handleDeleteUnit = (id: string, name: string) => {
    setConfirmDialog({
      title: "Delete Organizational Unit?",
      message: `Are you absolute certain you wish to delete "${name}"? All nested teams and personnel will be orphaned.`,
      onConfirm: async () => {
        try {
  ...
  const handleDeleteTeam = (id: string, name: string) => {
    setConfirmDialog({
      title: "Disband Team?",
      message: `Ensure you want to remove the team "${name}"?`,
      onConfirm: async () => {
        try {
  ...
  const handleExecuteUnitPromotion = async (
    staffId: string,
    unitId: string,
  ) => {
    ...
    const executePromote = async () => {
      try {
        await dispatch(
          updateUnit({ id: unitId, data: { ...targetUnit, headId: staffId } }),
        ).unwrap();
        showToast(
          `Smart Promotion Successful! Head of "${targetUnit.name}" is now ${candidate.name}.`,
        );
        setConfirmDialog(null);
        closeDrawer();
      } catch (err) {}
  ...
  const handleExecuteTeamPromotion = async (
    staffId: string,
    teamId: string,
  ) => {
    const candidate = staffMap.get(staffId);
    ...
    const executePromote = async () => {
      try {
        await dispatch(
          updateTeam({ id: teamId, data: { ...targetTeam, headId: staffId } }),
        ).unwrap();
        showToast(
          `Smart Promotion Successful! Team Lead of "${targetTeam.name}" is now ${candidate.name}.`,
        );
        setConfirmDialog(null);
        closeDrawer();
      } catch (err) {}
  ...
  const handleExecuteTransfer = async (
    ...
    try {
      await dispatch(
        updateUser({
          id: staffId,
          data: { ...candidate, unitId: destUnitId, teamId: destTeamId },
        }),
      ).unwrap();
      showToast(
        `Transferred "${candidate.name}" to ${destUnitId ? unitMap.get(destUnitId)?.name : "Unassigned"}.`,
      );
      closeDrawer();
    } catch (err) {}
  };
  ...
  const handleExportData = () => {
    ...
    a.setAttribute("download", `org_hierarchy_${Date.now()}.json`);
    ...
    fr.onload = (ev) => {
      try {
        ...
        if (
          ...
          showToast("Invalid JSON structure.", "warn");
        }
      } catch {
        showToast("Error reading file.", "warn");
      }
    };
    fr.readAsText(file);
    e.target.value = "";
  ...
  const handleResetToSeeds = () => {
    setConfirmDialog({
      ...
      message:
        "Are you sure you want to restore the original Demo Org structure?",
      onConfirm: () => {
  ...
  const triggerAddUnit = () => {
    setUnitForm({ name: "", headId: "", headName: "" });
    setDrawer({ type: "add-unit", title: "Establish New Unit" });
  };
  const triggerEditUnit = (unit: Unit) => {
    setUnitForm({
      name: unit.name,
      headId: unit.headId || "",
      headName: (unit as any).headName || "",
    });
    setDrawer({
      type: "edit-unit",
      title: `Modify ${unit.name} Unit`,
      targetId: unit.id,
    });
  };
  const triggerAddTeam = (prefillUnitId?: string) => {
    setTeamForm({
      name: "",
      unitId: prefillUnitId || units[0]?.id || "",
      headId: "",
      headName: "",
    });
  ...
  const triggerEditTeam = (team: Team) => {
    setTeamForm({
      ...
      unitId: team.unitId,
      headId: team.headId || "",
      headName: (team as any).headName || "",
    });
    setDrawer({
      type: "edit-team",
      title: `Modify ${team.name} Squad`,
      targetId: team.id,
    });
  };
  ...
  const triggerEditStaff = (personnel: Staff) => {
    ...
    setDrawer({
      type: "edit-staff",
      title: `Edit ${personnel.name}'s Profile`,
  ...
  const triggerUserPicker = async (
    ...
    try {
      const res = await axiosInstance.get(
        `/user/dropdown${roleFilter ? `?role=${roleFilter}` : ""}`,
      );
      if (res.data && res.data.data) {
        setPickerStaffList(
          res.data.data.map((u: any) => ({
            ...
            name: u.fullName,
            email: u.email,
            phone: u.contactNo ? String(u.contactNo) : "+1 (555) 000-0000",
            designation: u.role,
  ...
  const selectedUnit =
    selectedUnitId && selectedUnitId !== "unassigned"
      ? unitMap.get(selectedUnitId) || null
      : null;
  const selectedTeam = selectedTeamId
    ? teamMap.get(selectedTeamId) || null
    : null;
  ...
  const roleBadgeClasses = (role: string) => {
    ...
    if (r === "unit head" || r === "unit_head")
      return "bg-indigo-50 text-indigo-600 border border-indigo-100";
    if (r === "team head" || r === "team_head")
  ...
  const handleSelectUnit = (unitId: string) => {
    if (currentUser?.role === "team_head" && unitId !== currentUser.unit_id)
      return;
    if (currentUser?.role === "unit_head" && unitId !== currentUser.unit_id)
      return;
  ...
  const handleSelectTeam = (unitId: string, teamId: string) => {
    if (currentUser?.role === "team_head" && teamId !== currentUser.team_id)
      return;
    if (currentUser?.role === "unit_head" && unitId !== currentUser.unit_id)
      return;
    setSelectedUnitId(unitId);
  ...
  const handleSelectEntireOrg = () => {
    if (currentUser?.role === "team_head" || currentUser?.role === "unit_head")
      return;
    setSelectedUnitId(null);
    setSelectedTeamId(null);
    setMobileSidebarOpen(false);
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto py-2">
        ...
        {units.map((unit) => {
          ...
          const headName = unit.headId
            ? staffMap.get(unit.headId)?.name || (unit as any).headName
            : null;
            ...
            <div key={unit.id} className="mb-0.5">
              ...
              >
                <div className="flex items-center gap-1.5 min-w-0">
                  <button
                    ...
                    className="p-0.5 rounded text-slate-400 hover:text-slate-700 shrink-0"
                  >
                    {isExpanded ? (
                ...
                {headName && (
                  <span className="text-[10px] text-slate-400 font-medium shrink-0 truncate max-w-[80px]">
                    Head: {headName.split(" ")[0]}
                  </span>
                )}
              </div>
              ...
              {isExpanded && (
                <div className="ml-8 mr-2 space-y-0.5 mb-1">
                  {unitTeams.map((team) => {
                    const leadName = team.headId
                      ? staffMap.get(team.headId)?.name ||
                        (team as any).headName
                      : null;
    ...
    <>
      <div className="flex flex-col h-screen w-full overflow-hidden bg-white font-sans text-xs">
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            {confirmDialog && (
              <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <motion.div
                  ...
                  className="bg-white border border-slate-200 rounded-xl p-5 max-w-sm w-full shadow-2xl space-y-4"
                >
                  ...
                  <div className="flex justify-end gap-2 pt-1">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-md bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold cursor-pointer transition-colors"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      className="px-3 py-1.5 rounded-md bg-red-600 hover:bg-red-700 text-white font-bold cursor-pointer transition-colors"
                    >
                      Proceed
                    </button>
                  </div>
          ...
          <div
            className={`fixed top-0 left-0 bottom-0 z-50 w-72 bg-white border-r border-slate-200 flex flex-col transition-transform lg:hidden ${mobileSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
          >
            {renderSidebarContent()}
          </div>
            ...
            <div className="px-6 pt-5 pb-4 border-b border-slate-100 flex items-start justify-between gap-4 shrink-0">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="p-1.5 rounded-lg border border-slate-200 bg-white block lg:hidden hover:bg-slate-50 text-slate-500 shrink-0"
                >
                  ...
                  <p className="text-slate-400 text-[13px] mt-0.5">
                    ...
                    {selectedUnit && (
                      <>
                        ...
                        <span className="font-semibold text-slate-600">
                          {selectedUnit.headId
                            ? staffMap.get(selectedUnit.headId)?.name ||
                              (selectedUnit as any).headName ||
                              "Unknown"
                            : "None"}
                        </span>
                      </>
                    ...
                    {selectedTeam && (
                      <>
                        ...
                        <span className="font-semibold text-slate-600">
                          {selectedTeam.headId
                            ? staffMap.get(selectedTeam.headId)?.name ||
                              (selectedTeam as any).headName ||
                              "Unknown"
                            : "None"}
                        </span>
                      </>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <div className="relative mr-2">
                  ...
                  <input
                    ...
                    className="w-56 bg-white text-slate-800 placeholder-slate-400 border border-slate-200 rounded-lg py-2 pl-9 pr-3 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-[12px] shadow-sm"
                  />
                </div>
                {selectedUnit && !selectedTeam && (
                  <>
                    <button
                      onClick={() => triggerEditUnit(selectedUnit)}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white hover:bg-emerald-50 border border-slate-200 font-semibold text-slate-700 cursor-pointer transition-colors text-[12px]"
                    >
                    ...
                    <button
                      onClick={() =>
                        handleDeleteUnit(selectedUnit.id, selectedUnit.name)
                      }
                      className="p-2 rounded-lg bg-white hover:bg-red-50 hover:text-red-600 border border-slate-200 text-slate-400 cursor-pointer transition-colors"
                    >
                ...
                {selectedTeam && (
                  <>
                    <button
                      onClick={() => triggerEditTeam(selectedTeam)}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white hover:bg-emerald-50 border border-slate-200 font-semibold text-slate-700 cursor-pointer transition-colors text-[12px]"
                    >
                      <Edit2 className="w-3.5 h-3.5" />
                      <span>Edit Team</span>
                ...
                <button
                  onClick={() =>
                    triggerAddStaff(
                      selectedUnitId === "unassigned" || selectedUnitId === null
                        ? null
                        : selectedUnitId,
                      selectedTeamId,
                    )
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="pb-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 pr-4">
                      User
                    </th>
                    <th className="pb-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 pr-4">
                      Role
                    ...
                    <th className="pb-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {staff.length === 0 ? (
                    <tr>
                      <td
                        colSpan={6}
                        className="py-16 text-center text-slate-300 italic text-[13px]"
                      >
                        No staff members found for this view.
                      </td>
                    </tr>
                  ) : (
                      ...
                      return (
                        ...
                        >
                          <td className="py-3.5 pr-4">
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center font-bold text-[12px] uppercase shrink-0">
                          ...
                          <td className="py-3.5 pr-4">
                            <span
                              className={`inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-semibold ${roleBadgeClasses(role)}`}
                            >
                              {role}
                            </span>
                          </td>
                          <td className="py-3.5 pr-4 text-slate-600 text-[13px]">
                            {unit ? (
                              unit.name
                            ) : (
                          ...
                          <td className="py-3.5">
                            <div className="flex items-center justify-end gap-1">
                              <button
                                ...
                                className="p-1.5 rounded-lg text-slate-600 bg-slate-100 cursor-pointer transition-colors"
                              >
                                <Edit2 className="w-3.5 h-3.5" />
                              </button>
                              <button
                                ...
                                className="p-1.5 rounded-lg text-red-500 bg-red-50 cursor-pointer transition-colors"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                            </div>
                ...
                <div className="flex items-center justify-between border-t border-slate-100 py-4 mt-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 border border-slate-200 rounded-md text-[12px] font-medium text-slate-600 bg-white hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Previous
                    </button>
                    <button
                      onClick={() => setCurrentPage((p) => p + 1)}
                      disabled={currentPage * 10 >= pagination.total}
                      className="px-3 py-1.5 border border-slate-200 rounded-md text-[12px] font-medium text-slate-600 bg-white hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Next
                    </button>
            ...
            {drawer.type !== null && (
              <>
                ...
                <motion.div
                  ...
                  className="fixed right-0 top-0 bottom-0 z-50 w-full sm:w-[400px] bg-white flex flex-col shadow-2xl font-sans"
                >
                  {/* Drawer Header */}
                  <div className="bg-emerald-500 px-5 py-4 flex items-center justify-between shrink-0">
                    ...
                    <button
                      type="button"
                      onClick={closeDrawer}
                      className="p-1.5 rounded-full hover:bg-white/20 text-white/80 hover:text-white cursor-pointer transition-colors"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto bg-gray-100">
                    {/* Unit Form */}
                    {(drawer.type === "add-unit" ||
                      drawer.type === "edit-unit") && (
                      <form onSubmit={handleSaveUnit}>
                        <div className="bg-white mx-0 mt-3 mx-4 rounded-xl overflow-hidden shadow-sm">
                          <div className="px-4 pt-4 pb-2 space-y-4">
                            <div className="space-y-1">
                              ...
                              <input
                                ...
                                className="w-full bg-gray-100 text-slate-800 placeholder-slate-400 border-0 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
                              />
                            </div>
                            <div className="space-y-1">
                              <label className="text-[11px] font-semibold text-emerald-600 uppercase tracking-wider block">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="flex-1 bg-gray-100 rounded-lg px-3 py-2.5 text-sm text-slate-700 flex items-center justify-between min-w-0">
                                  <span className="truncate">
                                    {unitForm.headId ? (
                                      staffMap.get(unitForm.headId)?.name ||
                                      unitForm.headName || (
                                ...
                                <button
                                  ...
                                  className="px-3 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold rounded-lg shrink-0 cursor-pointer transition-colors"
                                >
                                  Choose
                                </button>
                              </div>
                          ...
                          <div className="px-4 pb-4 pt-2">
                            <button
                              type="submit"
                              className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg font-semibold cursor-pointer transition-colors text-sm"
                            >
                              Save Unit
                            </button>
                          </div>
                    ...
                    {/* Team Form */}
                    {(drawer.type === "add-team" ||
                      drawer.type === "edit-team") && (
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="flex-1 bg-gray-100 rounded-lg px-3 py-2.5 text-sm text-slate-700 flex items-center justify-between min-w-0">
                                  <span className="truncate">
                                    {teamForm.headId ? (
                                      staffMap.get(teamForm.headId)?.name ||
                                      teamForm.headName || (
                                        <span className="text-slate-400">
                                          Unknown Name
                                        </span>
                    ...
                    {/* Staff Form */}
                    {(drawer.type === "add-staff" ||
                      drawer.type === "edit-staff") && (
                      <form onSubmit={handleSaveStaff}>
                        {/* Avatar preview */}
                        <div className="flex justify-center pt-5 pb-2"></div>
                              ...
                              >
                                ...
                                {DESIGNATIONS.filter((d) => {
                                  if (currentUser?.role === "admin")
                                    return true;
                                  if (currentUser?.role === "unit_head")
                                    return ["team_head", "staff"].includes(
                                      d.value,
                                    );
                                  if (currentUser?.role === "team_head")
                                    return d.value === "staff";
                                  return false;
                                }).map((d) => (
                    ...
                    {drawer.type === "user-picker" && (
                      <div className="p-4 space-y-3">
                        {!isCreatingUserPicker ? (
                          <>
                            <div className="flex items-center justify-between">
                              ...
                              <button
                                onClick={() =>
                                  triggerAddStaff(
                                    drawer.payload?.unitId,
                                    drawer.payload?.teamId,
                                    drawer.payload?.roleFilter,
                                  )
                                }
                                className="flex items-center gap-1 text-[12px] text-emerald-600 hover:text-emerald-700 font-semibold cursor-pointer"
                              >
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                className="w-full bg-white text-slate-800 placeholder-slate-400 border border-slate-200 rounded-xl py-2.5 pl-9 pr-3 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm shadow-sm"
                              />
                            </div>
                            <div className="space-y-1.5 max-h-[420px] overflow-y-auto">
                              {pickerFilteredStaff.map((person) => {
                                const isChecked =
                                  drawer.payload?.currentSelectionId ===
                                  person.id;
                                return (
                                  <div
                                    key={person.id}
                                  ...
                                  >
                                    <div
                                      className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold uppercase shrink-0 ${isChecked ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-600"}`}
                                    >
                                      {person.name.charAt(0)}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      <p
                                        className={`text-sm font-semibold truncate ${isChecked ? "text-emerald-700" : "text-slate-800"}`}
                                      >
                                        {person.name}
                                      </p>
                          ...
                          <form
                            onSubmit={handleCreateUserInsidePicker}
                            className="space-y-3"
                          >
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <button
                                type="button"
                                onClick={() => setIsCreatingUserPicker(false)}
                                className="text-[12px] text-emerald-600 hover:text-emerald-700 font-semibold cursor-pointer"
                              >
                                ← Back
                              </button>
                            </div>
                          ...
                          <div className="p-4 space-y-3">
                            {person && (
                              <div className="bg-white rounded-xl p-4 flex items-center gap-3 shadow-sm">
                                <div className="w-12 h-12 rounded-full bg-emerald-500 text-white flex items-center justify-center text-lg font-bold uppercase shrink-0">
                                  {person.name.charAt(0)}
                                </div>
                                <div className="min-w-0">
                            ...
                            <div className="bg-white rounded-xl p-4 space-y-3 shadow-sm">
                              ...
                              <div className="space-y-1">
                                ...
                                <select
                                  value={drawer.payload?.teamId || ""}
                                  disabled={!drawer.payload?.unitId}
                                  onChange={(e) =>
                                    setDrawer((p) => ({
                                      ...p,
                                      payload: {
                                ...
                                >
                                  ...
                                  {drawer.payload?.unitId &&
                                    teams
                                      .filter(
                                        (t) =>
                                          t.unitId === drawer.payload.unitId,
                                      )
                                      .map((t) => (
                                        <option key={t.id} value={t.id}>
                                          {t.name}
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleExecuteTransfer(
                                    drawer.targetId!,
                                    drawer.payload?.unitId || null,
                                    drawer.payload?.teamId || null,
                                  )
                                }
                                className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg font-semibold cursor-pointer transition-colors text-sm"
                              >
                      ...
                      (() => {
                        ...
                        return (
                          <div className="p-4 space-y-3">
                            {person && (
                              <div className="bg-white rounded-xl p-4 flex items-center gap-3 shadow-sm">
                                <div className="w-12 h-12 rounded-full bg-amber-500 text-white flex items-center justify-center text-lg font-bold uppercase shrink-0">
                                  {person.name.charAt(0)}
                                </div>
                                <div className="min-w-0">
                                  <p className="font-semibold text-slate-800 text-sm truncate">
                                  ...
                                  <span className="inline-flex items-center gap-1 mt-0.5 text-[10px] font-semibold text-amber-600">
                                    <Crown className="w-3 h-3" /> Promoting to
                                    Lead
                                  </span>
                                </div>
                              </div>
                            ...
                            <div className="bg-white rounded-xl p-4 space-y-3 shadow-sm">
                              <div className="space-y-1">
                                ...
                                <select
                                  value={drawer.payload?.selectedTeamId || ""}
                                  onChange={(e) =>
                                    setDrawer((p) => ({
                                      ...p,
                                      payload: {
                                ...
                                >
                                  {teams.map((t) => (
                                    <option key={t.id} value={t.id}>
                                      {t.name} · {unitMap.get(t.unitId)?.name}
                                    </option>
                                  ))}
                                </select>
                              </div>
                              <button
                                type="button"
                                onClick={() =>
                                  handleExecuteTeamPromotion(
                                    drawer.targetId!,
                                    drawer.payload?.selectedTeamId,
                                  )
                                }
                                className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-semibold cursor-pointer transition-colors text-sm"
                              >
                                Confirm Promotion
                              </button>
                            </div>
